...
function issueAccess(user, clientId, sid) {
  ...
  // Не зависит от clientId: даже сервисный токен (Trip, Movies…) несёт этот
  // claim, если пользователь — админ. Так Admin может не отдельным логином,
  // а тем же SSO проверить право входа независимо от того, куда человек
  // авторизовался в моменте.
  if (user.is_admin) payload.admin = true;
