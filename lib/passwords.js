...
/** Отдельно от validateCreds — нужна и там, и при сбросе пароля по ссылке,
 *  где логина в запросе нет вовсе (личность подтверждает сам токен). */
...
/**
 * Имя — поле чисто косметическое (в отличие от логина/почты ни на что не
 * влияет), поэтому вместо ошибки молча приводим к приемлемому виду: та же
 * логика, что у локального "Отображаемого имени" в Finance. Пустая строка
 * нормализуется в null — это и есть состояние "имя не задано".
 */
function normalizeDisplayName(v) {
  const s = String(v || "").trim().replace(/\s+/g, " ").slice(0, MAX_DISPLAY_NAME);
  return s || null;
}

const RU_PHONE_RE = /^\+7\d{10}$/;
/**
 * Телефон — задел под будущую интеграцию с СБП, а она работает только с
 * российскими номерами, поэтому не претендуем на универсальность: приводим
 * привычные способы записи (8916…, 7916…, голый 916…) к единому +7XXXXXXXXXX.
 * В отличие от имени — поле не косметическое (в СБП попадёт как есть), молча
 * портить его подгонкой нельзя, поэтому неразобранное отбраковывает
 * validatePhone с понятной причиной, а не тихо режется.
 */
function normalizePhone(v) {
  const raw = String(v || "").trim();
  if (!raw) return null; // поле реально пустое — телефон просто не указан, это не ошибка

  let d = raw.replace(/[^\d+]/g, "").replace(/(?!^)\+/g, "");
  if (/^8\d{10}$/.test(d)) d = "+7" + d.slice(1);       // 8 916 123-45-67
  else if (/^7\d{10}$/.test(d)) d = "+" + d;             // 7 916 123-45-67 без плюса
  else if (/^9\d{9}$/.test(d)) d = "+7" + d;             // голый мобильный, без кода
  else if (d && !d.startsWith("+")) d = "+" + d;
  // Что-то ввели, но цифр там не нашлось ("abc") — возвращаем как есть, а не
  // null: иначе validatePhone решит, что телефон не указан, и молча проглотит
  // опечатку вместо явной ошибки.
  return d || raw;
}
function validatePhone(phone) {
  if (phone === null) return null;
  if (!RU_PHONE_RE.test(phone)) return "Телефон: российский номер вида +7 900 123-45-67";
  return null;
}
...
module.exports = {
  ...
  normalizeUsername, normalizeEmail, validateCreds, validatePassword, validateEmail, normalizeDisplayName,
