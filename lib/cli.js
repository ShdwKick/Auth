...
BurningHouse Auth — команды обслуживания
  ...
  make-admin  <логин>                       выдать доступ в Admin
  revoke-admin <логин>                      забрать доступ в Admin
...
function run(argv) {
  ...
  switch (cmd) {
    ...
    case "admins": {
      const rows = store.listUsers().filter(u => u.is_admin);
      if (!rows.length) return console.log("(пусто)");
      for (const u of rows) console.log(`${u.username.padEnd(20)}  ${u.id}`);
      break;
    }

    case "make-admin": case "revoke-admin": {
      const [login] = args;
      if (!login) fail(`Использование: node server.js ${cmd} <логин>`);
      const u = store.getUserByName(login);
      if (!u) fail("Пользователь не найден");
      store.setAdmin(u.id, cmd === "make-admin");
      // Уже выданные токены несут старое значение claim'а до истечения (макс.
      // ACCESS_TTL) — тот же компромисс, что и у disable/enable выше.
      console.log(`${u.username}: доступ в Admin ${cmd === "make-admin" ? "выдан" : "отозван"}`);
      break;
    }
