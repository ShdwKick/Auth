...
const stmt = {
  ...
  userByInviteToken: db.prepare("SELECT * FROM users WHERE invite_token = ?"),
  setInviteToken: db.prepare("UPDATE users SET invite_token = ? WHERE id = ?"),
  ...
  // Последняя активность — по любому из двух видов сессий: браузерной SSO-куке
  // или refresh-сессии сервиса. Кто чем пользовался в последний раз, для Admin
  // не важно, важен факт "было ли что-то вообще и когда" — так что берём максимум.
  listLastSeen: db.prepare(`
    SELECT user_id, MAX(last_seen) AS last_seen FROM (
      SELECT user_id, last_seen FROM sessions
      UNION ALL
      SELECT user_id, last_seen FROM sso_sessions
    ) GROUP BY user_id
  `),
  ...
  // friendships
  insertFriendship: db.prepare("INSERT INTO friendships (id, user_lo, user_hi, requested_by, created_at) VALUES (?, ?, ?, ?, ?)"),
  friendshipByPair: db.prepare("SELECT * FROM friendships WHERE user_lo = ? AND user_hi = ?"),
  friendshipById: db.prepare("SELECT * FROM friendships WHERE id = ?"),
  acceptFriendship: db.prepare("UPDATE friendships SET status = 'accepted', accepted_at = ? WHERE id = ?"),
  deleteFriendship: db.prepare("DELETE FROM friendships WHERE id = ? AND (user_lo = ? OR user_hi = ?)"),
  listAcceptedOf: db.prepare("SELECT * FROM friendships WHERE status = 'accepted' AND (user_lo = ? OR user_hi = ?) ORDER BY accepted_at DESC"),
  listIncomingOf: db.prepare("SELECT * FROM friendships WHERE status = 'pending' AND requested_by <> ? AND (user_lo = ? OR user_hi = ?) ORDER BY created_at DESC"),
  listOutgoingOf: db.prepare("SELECT * FROM friendships WHERE status = 'pending' AND requested_by = ? ORDER BY created_at DESC"),
};
...
/** { user_id: последний ts из любой сессии }. Кого нет в карте — не заходил ни разу. */
function lastSeenMap() {
  const map = {};
  for (const row of stmt.listLastSeen.all()) map[row.user_id] = row.last_seen;
  return map;
}
...
/** Генерируется лениво — до первого запроса колонка просто NULL. */
function getOrCreateInviteToken(userId) {
  const u = getUserById(userId);
  if (u && u.invite_token) return u.invite_token;
  const token = crypto.randomBytes(16).toString("hex");
  stmt.setInviteToken.run(token, userId);
  return token;
}
/** Старая ссылка сразу перестаёт работать — на случай, если ушла не тому. */
function regenerateInviteToken(userId) {
  const token = crypto.randomBytes(16).toString("hex");
  stmt.setInviteToken.run(token, userId);
  return token;
}
function getUserByInviteToken(token) {
  return token ? stmt.userByInviteToken.get(String(token)) || null : null;
}

/* ---------- друзья ---------- */

/** Каждому другу — своё видимое имя/телефон: имя видно другу ВСЕГДА (в отличие
 *  от computedName для сервисов, которая смотрит на show_display_name), а
 *  телефон — только если включена та же галочка, что и для сервисов
 *  (share_phone): одна настройка на оба случая, см. index.html/app.js. */
function friendView(u) {
  // userId, не id: спреднутое поле не должно тихо перебивать id самой заявки/
  // связи (friendshipId/requestId) в объектах ниже — их ключи разные не просто
  // так, порядок полей после ...friendView(u) иначе решал бы, что победит.
  return {
    userId: u.id, username: u.username,
    name: u.display_name || u.username,
    phone: (u.share_phone && u.phone) ? u.phone : null,
  };
}

function otherUserId(row, myId) { return row.user_lo === myId ? row.user_hi : row.user_lo; }

/**
 * Запрос дружбы (по логину, из кабинета). Если встречный запрос уже есть —
 * ведёт себя как принятие, а не как вторая заявка (UNIQUE(user_lo, user_hi)
 * всё равно не дал бы вставить вторую строку на ту же пару).
 * Возвращает { ok, accepted } либо { error }.
 */
function sendFriendRequest(fromId, toId) {
  if (fromId === toId) return { error: "self" };
  const [lo, hi] = [fromId, toId].sort();
  const existing = stmt.friendshipByPair.get(lo, hi);
  if (existing) {
    if (existing.status === "accepted") return { error: "already_friends" };
    if (existing.requested_by === fromId) return { error: "already_requested" };
    stmt.acceptFriendship.run(Date.now(), existing.id);
    return { ok: true, accepted: true, id: existing.id };
  }
  const id = crypto.randomUUID();
  stmt.insertFriendship.run(id, lo, hi, fromId, Date.now());
  return { ok: true, accepted: false, id };
}

/** Принять ВХОДЯЩУЮ заявку — тот, кто её прислал, не может принять сам себя. */
function acceptFriendRequest(id, userId) {
  const row = stmt.friendshipById.get(String(id || ""));
  if (!row || row.status !== "pending") return { error: "not_found" };
  if (row.user_lo !== userId && row.user_hi !== userId) return { error: "not_found" };
  if (row.requested_by === userId) return { error: "cant_accept_own" };
  stmt.acceptFriendship.run(Date.now(), row.id);
  return { ok: true };
}

/** Одна и та же операция для «отклонить входящую», «отменить исходящую» и
 *  «удалить из друзей» — везде просто убрать строку связи. */
function removeFriendship(id, userId) {
  const r = stmt.deleteFriendship.run(String(id || ""), userId, userId);
  return r.changes > 0;
}

/** Переход по персональной ссылке — сразу дружба, без отдельного принятия
 *  (см. обсуждение в чате: «перешёл и вошёл — сразу друг», как у Trip). */
function acceptFriendInvite(myId, ownerId) {
  if (myId === ownerId) return { error: "self" };
  const [lo, hi] = [myId, ownerId].sort();
  const existing = stmt.friendshipByPair.get(lo, hi);
  const now = Date.now();
  if (existing) {
    if (existing.status === "accepted") return { error: "already_friends" };
    stmt.acceptFriendship.run(now, existing.id);
    return { ok: true };
  }
  const id = crypto.randomUUID();
  stmt.insertFriendship.run(id, lo, hi, myId, now);
  stmt.acceptFriendship.run(now, id);
  return { ok: true };
}

function listFriends(userId) {
  return stmt.listAcceptedOf.all(userId, userId).map(row => {
    const u = getUserById(otherUserId(row, userId));
    return u ? { friendshipId: row.id, since: row.accepted_at, ...friendView(u) } : null;
  }).filter(Boolean);
}
function listIncomingRequests(userId) {
  return stmt.listIncomingOf.all(userId, userId, userId).map(row => {
    const u = getUserById(row.requested_by);
    return u ? { requestId: row.id, since: row.created_at, ...friendView(u) } : null;
  }).filter(Boolean);
}
function listOutgoingRequests(userId) {
  return stmt.listOutgoingOf.all(userId).map(row => {
    const u = getUserById(otherUserId(row, userId));
    return u ? { requestId: row.id, since: row.created_at, ...friendView(u) } : null;
  }).filter(Boolean);
}
...
module.exports = {
  ...
  setAdmin, setDisabled, lastSeenMap,
  ...
  getOrCreateInviteToken, regenerateInviteToken, getUserByInviteToken,
  sendFriendRequest, acceptFriendRequest, removeFriendship, acceptFriendInvite,
