...
const stmt = {
  ...
  updatePhone: db.prepare("UPDATE users SET phone = ?, phone_verified = 0 WHERE id = ?"),
...
function createUser(username, password, email = null, displayName = null, showDisplayName = false, phone = null) {
  ...
  if (phone) stmt.updatePhone.run(phone, id);
...
/** phone — уже нормализованное значение или null, чтобы очистить. */
function setPhone(userId, phone) {
  stmt.updatePhone.run(phone, userId);
}
...
/* Телефон в ownProfile тоже сырой (без учёта верификации) — как и имя,
   это данные "о себе", отдаются только владельцу аккаунта. В publicUser()
   (то, что видят другие сервисы) телефон намеренно не попадает вовсе —
   не паспортные данные для сторонних сервисов, задел под будущий СБП живёт
   внутри самого auth. */
const ownProfile = u => ({
...
module.exports = {
  sha256,
  getUserById, getUserByName, getUserByEmail, getUserByPhone, createUser, importUser,
  setPassword, setEmail, setPhone, setProfile, checkPassword, publicUser, ownProfile,
